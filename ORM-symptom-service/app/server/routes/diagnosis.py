...
async def add_diagnosis_data(request: Request, diagnosis: Diagnosis_schema = Body(...)):
    ...
    new_diagnosis = await add_diagnosis(request.app.database['diagnosiss'], diagnosis)
    return ResponseModel(new_diagnosis, "Diagnosis added successfully.")

@router.get("/", response_description="Diagnosiss retrieved")
async def get_diagnosiss_data(request: Request):
    diagnosiss = await retrieve_diagnosiss(request.app.database['diagnosiss'])
    diagnosiss_list = list()
    if diagnosiss:
        for diagnosis in diagnosiss:
            # diagnosis_dict = await diagnosis_list_to_dict(diagnosis)
            diagnosiss_list.append(diagnosis)
        return diagnosiss_list
...
async def get_diagnosis_data(request: Request, id: str):
    diagnosis = await retrieve_diagnosis_by_id(request.app.database['diagnosiss'], id)
    return diagnosis
...
async def update_diagnosis_data(request: Request, id: str, req: Update_diagnosis_schema = Body(...)):
    ...
    if len(diagnosis) >= 1:
        update_result = await update_diagnosis(request.app.database["diagnosiss"], id, diagnosis)
    ...
    if (
        existing_diagnosis := await retrieve_diagnosis_by_id(request.app.database['diagnosiss'], id)
    ) is not None:
...
async def delete_diagnosis_data(request: Request, id: str):
    deleted_diagnosis = await delete_diagnosis(request.app.database['diagnosiss'], id)
